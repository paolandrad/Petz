...
class LoginScreen(Screen):
    ...
    def login_verification(self):
        ...
        if self.ids.email.text == 'petz@gmail.com' and self.ids.passw.text == '1234':
            ...
            App.get_running_app().root.current = 'petzmenu'
        else:
    ...
    def incorrect_login(self):
        ...
        self.ids.dica.text = 'email = petz@gmail.com\nsenha = 1234'  # Mostra qual o login da conta
...
class PetzHome(Screen):
    # Screen do Menu inicial da Petz
...
class Banner(BoxLayout):
    # Banner em destaque (Adote um Amigo!)
    pass
...
class ResultPage(Screen):
    ...
    def pre_search(self, text):
        if len(text) > 0:
            # Pega todos os cachorros e gatos disponíveis sem repetir
            all_images = []
...
class Petz(App):
    ...
    app_icon = os.path.join('config', 'logo-petz.png')
    petz_logo = os.path.join('config', 'logo-petz.png')
    background_login = os.path.join('config', 'petz.webp')
    dropmenu_icon = os.path.join('config', 'menu-amarelo.png')
    search_btn = os.path.join('config', 'search_btn.png')
    roundborder = os.path.join('config', 'roundborder.png')
    banner = os.path.join('config', 'pet1.jpg')
    petz_font = os.path.join('config', 'net_font.ttf')
